  ...
  <v-card elevation="0" class="rounded-0">
    <v-card-text>
      <v-row>
        ...
        <v-col cols="2">
          {{ visit.visit_class ? visitClasses.find(cl => cl.value === visit.visit_class).label : '' }}
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="2" class="font-weight-bold">
        ...
        <v-col cols="2">
          {{ visit.study_epoch_uid ? epochs.find(epoch => epoch.uid === visit.study_epoch_uid).epoch_name : '' }}
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="2" class="font-weight-bold">
      ...
      <v-row>
        ...
        <v-col cols="2">
          {{ visit.is_global_anchor_visit|yesno }}
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="2" class="font-weight-bold">
      ...
      <v-row>
        ...
        <v-col cols="2">
          {{ visit.min_visit_window_value }} / {{ visit.max_visit_window_value }} {{ visit.visit_window_unit_name }}
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="2" class="font-weight-bold">
...
export default {
  ...
  mounted () {
    studyEpochs.getStudyVisit(this.$route.params.study_id, this.$route.params.id).then(resp => {
