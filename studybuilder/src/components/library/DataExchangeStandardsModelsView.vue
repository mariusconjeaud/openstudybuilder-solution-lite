        ...
        <v-card-text>
          <v-row>
            ...
            <v-col cols="3">
              ...
              <p class="font-weight-regular">{{ activeModel.start_date ? activeModel.start_date.substring(0, 10) : '' }}</p>
            </v-col>
            <v-col cols="3">
              <div class="font-weight-bold">{{ $t('DataModels.implemented_by') }}</div>
              <div v-for="guide of activeModel.implementation_guides" :key="guide.name">
                <a href="#" @click="redirectToGuide(guide)" class="font-weight-regular">{{ guide.name }}</a>
              </div>
            </v-col>
          </v-row>
        </v-card-text>
  ...
  watch: {
    ...
    redirectModel (value) {
      if (value && value.implementation) {
        ...
        this.activeModel = this.models.find(model => model.name === value.implementation.name)
        this.tab = this.datasetClasses.findIndex(el => el.label === value.data[0].dataset_class.dataset_class_name)
