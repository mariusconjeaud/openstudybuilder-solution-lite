...
from mdr_standards_import.cdisc_ct.inconsistency_resolver import InconsistencyResolver

from mdr_standards_import.cdisc_ct.repositories.repository import await_indexes, create_import_node, create_indexes_if_not_existent, create_ct_import
...
def print_summary(tx, import_id, start_time):
    ...
    print(f"==")
    print(f"== Summary for the import into the CDISC CT DB:")
    print(f"==")
    print(f"==    # resolved inconsistencies: {result.get('num_resolved', 'n/a')}")
    print(f"== ! # remaining inconsistencies: {result.get('num_inconsistencies', 'n/a')}")
    print(f"==")
...
def import_json_data_into_cdisc_ct_db(effective_date, data_directory, cdisc_import_neo4j_driver, cdisc_import_db_name, user_initials):
    """
    ...
    :param cdisc_import_neo4j_driver: the Neo4j driver instance that is used in order to connect to the Neo4j DBMS
        that holds the cdisc_ct-import database (cf. cdisc_import_db_name);
    ...
    :param mdr_neo4j_driver: the Neo4j driver instance that is used in order to connect to the Neo4j DBMS
        that holds the mdr database (cf. mdr_db_name);
    :param mdr_db_name: string, the name of the Neo4j database in which the CDISC data will eventually be stored
    ...
    try:
        ...
        with cdisc_import_neo4j_driver.session(database="system") as session:
            session.run("CREATE DATABASE $database IF NOT EXISTS", database=cdisc_import_db_name)
        ...
        with cdisc_import_neo4j_driver.session(database=cdisc_import_db_name) as session:
            ...
            file_names = [file_name for file_name in listdir(data_directory) if
                            file_name.endswith(effective_date + ".json")
                            ]
        ...
        print(f"== The `Import` node already exists. Skipping the import for the effective_date='{effective_date}'.")
