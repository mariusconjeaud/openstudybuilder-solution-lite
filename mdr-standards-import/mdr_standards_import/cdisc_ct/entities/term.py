...
from mdr_standards_import.cdisc_ct.term_submission_value_differentiator import TermSubmissionValueDifferentiator
...
class Term:
    ...
    def add_attributes_set(self, term_attributes_set: 'set[TermAttributes]', codelist, package):
    ...
    def add_attributes(self, term_attributes: TermAttributes, codelist, package):
        ...
        for attributes in self.__attributes_set:
            if term_attributes == attributes:  # this triggers a call to CodelistAttributes.__eq__()
    ...
    def get_inconsistencies(self, user_initials: str):
        ...
        if not self.has_consistent_submission_values():
            ...
            inconsistency = Inconsistency(
                Inconsistency.inconsistent_term_submission_value_tagline, message, user_initials)
        ...
        if len(self.__attributes_set) > 1:
            ...
            inconsistency = Inconsistency(
                Inconsistency.inconsistent_term_attributes_tagline, message, user_initials)
            inconsistency.set_affected_term(self)
            inconsistencies.append(inconsistency)
    ...
    def fork_term_based_on_code_submission_values(self, package) -> Sequence[T]:
        pairs: Sequence[TermSubmissionValuePair] = self.get_code_name_submission_value_pairs()
    ...
    def fork_for_code_submission_value(self, code_submission_value: str, tsv_pairs: Sequence[TermSubmissionValuePair], package) -> T:
        ...
        for tsv_pair in tsv_pairs:
            ...
            if tsv_pair.is_single():
                ...
                if tsv_pair.is_name_tsv_consistent():
                    ...
                    for nsv in tsv_pair.get_name_submission_values():
                        attr_set = self.__get_attributes_set(
                            name_codelist, nsv)
                        term.add_attributes_set(
                            attr_set, name_codelist, package)
                    attr_set = self.__get_attributes_set(code_codelist)
                    term.add_attributes_set(attr_set, code_codelist, package)
            ...
            codelists = (tsv_pair.get_code_codelist(),
                         tsv_pair.get_name_codelist())
            for codelist in codelists:
                for submission_value in self.__get_submission_values_for_codelist(codelist):
                    term.add_submission_value(
                        submission_value, codelist, package)
    ...
    def get_code_name_submission_value_pairs(self) -> Sequence[TermSubmissionValuePair]:
        ...
        submission_values: Sequence[CodelistTermSubmissionValue] = self.__get_codelist_term_submission_values(
        )
    ...
    def __find_term_submission_value_pair(self, tsv1: CodelistTermSubmissionValue, other_tsvs: Sequence[CodelistTermSubmissionValue]):
        index = 0
        for tsv2 in other_tsvs:
            try:
                submission_value_pair = TermSubmissionValueDifferentiator.get_term_submission_value_pair(
                    tsv1, tsv2)
    ...
    def __get_codelist_term_submission_values(self) -> Sequence[CodelistTermSubmissionValue]:
        """
        ...
        """
        codelist_term_submission_values: dict[str,
                                              CodelistTermSubmissionValue] = dict()
        for submission_value in self.__submission_values.values():
            for codelist in submission_value.get_codelists():
                codelist_term_submission_value = codelist_term_submission_values.get(codelist.concept_id, CodelistTermSubmissionValue(
                    codelist))
                codelist_term_submission_values[codelist.concept_id] = codelist_term_submission_value
