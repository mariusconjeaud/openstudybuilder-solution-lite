...
class Codelist:
    ...
    def add_attributes(self, codelist_attributes: CodelistAttributes, package):
        ...
        for attributes in self.__attributes_set:
            if codelist_attributes == attributes:  # this calls .__eq__ on the CodelistAttributes
    ...
    def get_inconsistencies(self, user_initials: str):
        ...
        if len(self.__attributes_set) > 1:
            ...
            inconsistency = Inconsistency(
                Inconsistency.inconsistent_codelist_attributes_tagline, message, user_initials)
    ...
    def get_submission_value(self):
        ...
        for attributes in self.__attributes_set:
            if submission_value is None and attributes.submission_value is not None:
                ...
                continue
            if submission_value is not None and submission_value != attributes.submission_value:
                raise Exception()
    ...
    def is_name_ok_for_single_codelist(self) -> bool:
        ...
        """
        return self.get_name() is not None and not (self.get_name().lower().endswith(' name') or self.get_name().lower().endswith(' code'))
