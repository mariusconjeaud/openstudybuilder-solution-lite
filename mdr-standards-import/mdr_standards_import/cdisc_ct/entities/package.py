...
from mdr_standards_import.cdisc_ct.utils import string_to_boolean
...
class Package:
    ...
    def get_term_concept_ids_for_codelist(self, codelist: Codelist) -> 'list[str]':
        return [term.concept_id for term in self.__codelist_terms_map.get(codelist.concept_id, set())]
    ...
    def __set_attributes(self, name: str, effective_date: str, registration_status: str, label: str, description: str, source: str, href: str):
        self.set_name(name)
    ...
    def set_catalogue_name(self, name):
        self.catalogue_name: str = name[:-
                                        11].upper() if name is not None else None
    ...
    def __load_terms_data(self, terms_json_data, codelist: Codelist):
        for json in terms_json_data:
            ...
            term.add_submission_value(
                json.get("submissionValue", None), codelist, self)
            term.add_attributes(
                TermAttributes(
    ...
    def __differentiate_between_code_name_submission_values(self):
        for preliminary_term in self.__preliminary_terms.values():
            ...
            for forked_term in preliminary_term.fork_term_based_on_code_submission_values(self):
    ...
    def get_inconsistencies(self, user_initials: str):
        ...
        if len(self.__codelists) == 0:
            ...
            inconsistencies.append(Inconsistency(
                Inconsistency.no_codelists_in_package_tagline, message, user_initials))
