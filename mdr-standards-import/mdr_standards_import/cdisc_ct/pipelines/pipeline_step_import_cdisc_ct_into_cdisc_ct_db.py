"""
This script serves as the entry point for the pipeline step 'Import CDISC CT into CDISC CT DB'.

This script is meant to be called e.g. via:

> `python -m pipenv run pipeline_step_import_cdisc_ct_into_cdisc_ct_db <user initials> <JSON directory name> <skip download step>`
...
from mdr_standards_import.cdisc_ct.utils import get_directory_name, get_skip_download_step, get_user_initials


wrapper_import_cdisc_ct_into_cdisc_ct_db(get_user_initials(1), get_directory_name(2), get_skip_download_step(3))
