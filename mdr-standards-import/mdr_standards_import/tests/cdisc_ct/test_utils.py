from mdr_standards_import.cdisc_ct.utils import *
...
class Test:
    ...
    def test__get_sentence_case_string(self):
        ...
        assert get_sentence_case_string('Secondary Outcome Measure') == 'secondary outcome measure'
        # multiple words including acronyms (all uppercase letters) in the middle
        assert get_sentence_case_string('Prolonged ST Segment by ECG Finding') == 'prolonged ST segment by ECG finding'
        ...
        assert get_sentence_case_string('CDISC ADaM Analysis Reason Terminology') == 'CDISC ADaM analysis reason terminology'

        assert get_sentence_case_string('6MWT - Distance at 6 Minutes') == '6MWT - distance at 6 minutes'
