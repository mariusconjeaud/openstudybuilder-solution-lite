...
class StudyDiseaseMilestoneRepository:
    ...
    def create_query_filter_statement_neomodel(
        ...
        if study_uid:
            q_filters.append(Q(has_study_disease_milestone__study_root__uid=study_uid))
        return q_filters
    ...
    def _update(
        ...
        if create:
            ...
            new_study_disease_milestone.has_study_disease_milestone.connect(study_value)
        else:
            previous_item = StudyDiseaseMilestone.nodes.filter(uid=item.uid).has(
                has_study_disease_milestone=True
            )[0]
            if delete is False:
                # update
                ...
                new_study_disease_milestone.has_study_disease_milestone.connect(
                    study_value
                )
            else:
                # delete
            ...
            previous_item.has_study_disease_milestone.disconnect(study_value)
        return item
