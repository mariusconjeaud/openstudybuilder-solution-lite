...
@pytest.fixture(scope="module")
def test_data():
    inject_and_clear_db("old.json.test.study.week")
    library_service.create(name="Sponsor", is_editable=True)
    db.cypher_query(CREATE_BASE_TEMPLATE_PARAMETER_TREE)

    yield

    drop_db("old.json.test.study.week")


def test_post_study_week_which_is_a_tp(api_client):
    data = {
        "value": 1.23,
...
def test_post_study_week_which_is_a_tp_existing_study_week_is_returned(api_client):
    data = {
        "value": 1.23,
        "definition": "study_week_definition1",
        "abbreviation": "abbv",
...
def test_post_study_week_which_is_not_tp(api_client):
    data = {
        "value": 1.234,
    ...
    assert res["uid"] == "StudyWeek_000002"
    assert res["name"] == "Week 1.234"
    assert res["value"] == 1.234
    assert res["name_sentence_case"] == "week 1.234"
    assert res["definition"] == "study_week_definition2"
    assert res["abbreviation"] == "abbv"
...
def test_get_all_study_weeks(api_client):
    response = api_client.get("/concepts/study-weeks?total_count=true")

    assert_response_status_code(response, 200)

    res = response.json()

    assert res["items"][0]["uid"] == "StudyWeek_000001"
    assert res["items"][0]["name"] == "Week 1.23"
    assert res["items"][0]["value"] == 1.23
    assert res["items"][0]["name_sentence_case"] == "week 1.23"
    assert res["items"][0]["definition"] == "study_week_definition1"
    ...
    assert res["items"][1]["uid"] == "StudyWeek_000002"
    assert res["items"][1]["name"] == "Week 1.234"
    assert res["items"][1]["value"] == 1.234
    assert res["items"][1]["name_sentence_case"] == "week 1.234"
    assert res["items"][1]["definition"] == "study_week_definition2"
