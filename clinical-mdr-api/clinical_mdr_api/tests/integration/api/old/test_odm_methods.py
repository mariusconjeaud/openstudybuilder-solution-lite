...
@pytest.fixture(scope="module")
def test_data():
    inject_and_clear_db("old.json.test.odm.methods")
    ...
    drop_db("old.json.test.odm.methods")
...
def test_creating_a_new_odm_method(api_client):
    data = {
...
def test_updating_an_existing_odm_method(api_client):
    data = {
        "library_name": "Sponsor",
        "name": "name10",
        "oid": "oid1",
...
def test_create_a_new_odm_method_for_deleting_it(api_client):
    data = {
        "library_name": "Sponsor",
        "name": "name - delete",
        "oid": "oid2",
    ...
    assert res["uid"] == "OdmMethod_000002"
    assert res["name"] == "name - delete"
    assert res["library_name"] == "Sponsor"
    assert res["oid"] == "oid2"
    assert res["method_type"] == "type2"
    assert res["end_date"] is None
    assert res["status"] == "Draft"
    assert res["version"] == "0.1"
    assert res["change_description"] == "Initial version"
...
def test_creating_a_new_odm_method_with_relations(api_client):
    data = {
        "library_name": "Sponsor",
        "name": "string",
        "oid": "string",
    ...
    assert res["uid"] == "OdmMethod_000003"
    assert res["name"] == "string"
    assert res["library_name"] == "Sponsor"
    assert res["oid"] == "string"
    assert res["method_type"] == "string"
    assert res["end_date"] is None
    assert res["status"] == "Draft"
    assert res["version"] == "0.1"
    assert res["change_description"] == "Initial version"
...
def test_updating_an_existing_odm_method_with_relations(api_client):
    data = {
        "library_name": "Sponsor",
        "name": "name10",
        "oid": "oid1",
