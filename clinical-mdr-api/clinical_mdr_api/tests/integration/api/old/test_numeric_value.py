...
@pytest.fixture(scope="module")
def test_data():
    inject_and_clear_db("old.json.test.numeric.value")
    library_service.create(name="Sponsor", is_editable=True)
    db.cypher_query(CREATE_BASE_TEMPLATE_PARAMETER_TREE)

    yield

    drop_db("old.json.test.numeric.value")


def test_post_numeric_value_which_is_a_tp(api_client):
    data = {
        "value": 1.23,
    ...
    response = api_client.post("/concepts/numeric-values", json=data)

    assert_response_status_code(response, 201)

    res = response.json()

    assert res["uid"] == "NumericValue_000001"
    assert res["name"] == "1.23"
    assert res["value"] == 1.23
    assert res["name_sentence_case"] == "1.23"
...
def test_post_numeric_value_which_is_a_tp_existing_numeric_value_is_returned(
    api_client,
):
    data = {
        "value": 1.23,
...
def test_post_numeric_value_which_is_not_tp(api_client):
    data = {
        "value": 1.234,
    ...
    assert res["uid"] == "NumericValue_000002"
    assert res["name"] == "1.234"
    assert res["value"] == 1.234
    assert res["name_sentence_case"] == "1.234"
...
def test_get_all_numeric_values(api_client):
    response = api_client.get("/concepts/numeric-values?total_count=true")

    assert_response_status_code(response, 200)

    res = response.json()

    assert res["items"][0]["uid"] == "NumericValue_000001"
    assert res["items"][0]["name"] == "1.23"
    assert res["items"][0]["value"] == 1.23
    assert res["items"][0]["name_sentence_case"] == "1.23"
    ...
    assert res["items"][1]["uid"] == "NumericValue_000002"
    assert res["items"][1]["name"] == "1.234"
    assert res["items"][1]["value"] == 1.234
    assert res["items"][1]["name_sentence_case"] == "1.234"
