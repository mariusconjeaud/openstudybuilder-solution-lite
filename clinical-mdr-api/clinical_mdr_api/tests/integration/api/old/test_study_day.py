...
@pytest.fixture(scope="module")
def test_data():
    inject_and_clear_db("old.json.test.study.day")
    library_service.create(name="Sponsor", is_editable=True)
    db.cypher_query(CREATE_BASE_TEMPLATE_PARAMETER_TREE)

    yield

    drop_db("old.json.test.study.day")


def test_post_study_day_which_is_a_tp(api_client):
    data = {
        "value": 1.23,
    ...
    response = api_client.post("/concepts/study-days", json=data)

    assert_response_status_code(response, 201)

    res = response.json()

    assert res["uid"] == "StudyDay_000001"
    assert res["name"] == "Day 1.23"
    assert res["value"] == 1.23
    assert res["name_sentence_case"] == "day 1.23"
    assert res["definition"] == "study_day_definition1"
...
def test_post_study_day_which_is_a_tp_existing_study_day_is_returned(api_client):
    data = {
        "value": 1.23,
        "definition": "study_day_definition1",
        "abbreviation": "abbv",
...
def test_post_study_day_which_is_not_tp(api_client):
    data = {
        "value": 1.234,
    ...
    assert res["uid"] == "StudyDay_000002"
    assert res["name"] == "Day 1.234"
    assert res["value"] == 1.234
    assert res["name_sentence_case"] == "day 1.234"
    assert res["definition"] == "study_day_definition2"
    assert res["abbreviation"] == "abbv"
...
def test_get_all_study_days(api_client):
    response = api_client.get("/concepts/study-days?total_count=true")

    assert_response_status_code(response, 200)

    res = response.json()

    assert res["items"][0]["uid"] == "StudyDay_000001"
    assert res["items"][0]["name"] == "Day 1.23"
    assert res["items"][0]["value"] == 1.23
    assert res["items"][0]["name_sentence_case"] == "day 1.23"
    assert res["items"][0]["definition"] == "study_day_definition1"
    ...
    assert res["items"][1]["uid"] == "StudyDay_000002"
    assert res["items"][1]["name"] == "Day 1.234"
    assert res["items"][1]["value"] == 1.234
    assert res["items"][1]["name_sentence_case"] == "day 1.234"
    assert res["items"][1]["definition"] == "study_day_definition2"
