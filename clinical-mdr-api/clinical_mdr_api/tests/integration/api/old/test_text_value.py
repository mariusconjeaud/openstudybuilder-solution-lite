...
@pytest.fixture(scope="module")
def test_data():
    inject_and_clear_db("old.json.test.text.value")
    library_service.create(name="Sponsor", is_editable=True)
    db.cypher_query(CREATE_BASE_TEMPLATE_PARAMETER_TREE)

    yield

    drop_db("old.json.test.text.value")


def test_post_text_value_which_is_a_tp(api_client):
    data = {
    ...
    response = api_client.post("/concepts/text-values", json=data)

    assert_response_status_code(response, 201)

    res = response.json()

    assert res["uid"] == "TextValue_000001"
    assert res["name"] == "Text_value_name"
    assert res["name_sentence_case"] == "text_value_name"
    assert res["definition"] == "text_value_definition1"
...
def test_post_text_value_which_is_a_tp_existing_text_value_is_returned(api_client):
    data = {
        "name": "Text_value_name",
        "name_sentence_case": "text_value_name",
        "definition": "text_value_definition1",
...
def test_post_text_value_which_is_not_tp(api_client):
    data = {
    ...
    assert res["uid"] == "TextValue_000002"
    assert res["name"] == "Text_value_name2"
    assert res["name_sentence_case"] == "text_value_name2"
    assert res["definition"] == "text_value_definition2"
    assert res["abbreviation"] == "abbv"
...
def test_get_all_text_values(api_client):
    response = api_client.get("/concepts/text-values?total_count=true")

    assert_response_status_code(response, 200)

    res = response.json()

    assert res["items"][0]["uid"] == "TextValue_000001"
    assert res["items"][0]["name"] == "Text_value_name"
    assert res["items"][0]["name_sentence_case"] == "text_value_name"
    assert res["items"][0]["definition"] == "text_value_definition1"
    ...
    assert res["items"][1]["uid"] == "TextValue_000002"
    assert res["items"][1]["name"] == "Text_value_name2"
    assert res["items"][1]["name_sentence_case"] == "text_value_name2"
    assert res["items"][1]["definition"] == "text_value_definition2"
