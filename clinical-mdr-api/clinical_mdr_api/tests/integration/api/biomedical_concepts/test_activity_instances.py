...
def test_data():
    ...
    activity_items = [
        TestUtils.create_activity_item(
            name="Activity Item name A",
            activity_item_class_uid=activity_item_classes[0].uid,
            ct_term_uid=ct_terms[0].term_uid,
        ),
        TestUtils.create_activity_item(
            name="Activity Item name B",
            activity_item_class_uid=activity_item_classes[1].uid,
            ct_term_uid=ct_terms[1].term_uid,
        ),
    ]
...
def test_get_activity_instance(api_client):
    ...
    assert res["activity_items"][0]["name"] == activity_items[0].name
    assert (
        res["activity_items"][0]["activity_item_class_uid"]
        == activity_item_classes[0].uid
    )
    assert (
        res["activity_items"][0]["activity_item_class_name"]
...
def test_edit_activity_instance(api_client):
    ...
    assert res["activity_items"][0]["uid"] == activity_items[0].uid
    assert res["activity_items"][1]["uid"] == activity_items[1].uid
...
def test_post_activity_instance(api_client):
    ...
    assert res["name_sentence_case"] is None
...
def verify_instance_overview_content(res: dict):
    ...
    # activity items
    assert len(res["activity_items"]) == 2
    assert res["activity_items"][0]["name"] == "Activity Item name A"
    assert res["activity_items"][0]["ct_term_name"] == "Activity item term"
    assert res["activity_items"][0]["unit_definition_name"] is None
    assert (
    ...
    assert res["activity_items"][1]["ct_term_name"] == "Activity item term2"
    assert res["activity_items"][1]["unit_definition_name"] is None
    assert (
...
def verify_activity_overview_content(res: dict):
    ...
    assert res["activity"]["name_sentence_case"] is None
    assert res["activity"]["definition"] is None
