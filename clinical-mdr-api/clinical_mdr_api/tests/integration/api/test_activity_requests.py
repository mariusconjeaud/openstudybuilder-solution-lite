...
def test_data():
    ...
    inject_and_clear_db("activityrequests.api")
    global study_uid
    ...
    activity_requests_all = [
        TestUtils.create_activity_request(
            name="name A",
            request_rationale="New activity request rationale",
            approve=False,
        ),
        TestUtils.create_activity_request(name="name-AAA", approve=False),
        TestUtils.create_activity_request(name="name-BBB", approve=False),
        TestUtils.create_activity_request(
            name="name XXX", definition="def-XXX", approve=False
        ),
        TestUtils.create_activity_request(
            name="name YYY", definition="def-YYY", approve=False
        ),
    ]

    for index in range(5):
        activity_requests_all.append(
            TestUtils.create_activity_request(name=f"name-AAA-{index}", approve=False)
        )
        activity_requests_all.append(
            TestUtils.create_activity_request(name=f"name-BBB-{index}", approve=False)
        )
        activity_requests_all.append(
            TestUtils.create_activity_request(
                name=f"name-XXX-{index}", definition=f"def-XXX-{index}", approve=False
            )
        )
        activity_requests_all.append(
            TestUtils.create_activity_request(
                name=f"name-YYY-{index}", definition=f"def-YYY-{index}", approve=False
            )
        )
...
def test_edit_activity_request(api_client):
    activity_request = TestUtils.create_activity_request(
