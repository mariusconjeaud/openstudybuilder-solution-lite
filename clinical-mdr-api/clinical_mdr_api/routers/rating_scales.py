...
from clinical_mdr_api.services.concepts.activities.rating_scale_service import (
    RatingScaleService,
)

router = APIRouter()

RatingScaleUID = Path(None, description="The unique id of the rating scale")


@router.get(
    "/rating-scales",
    summary="List all rating scales (for a given library)",
    description="""
State before:
...
Possible errors:
 - Invalid library name specified.""",
    response_model=CustomPage[RatingScale],
    status_code=200,
...
def get_activities(
    ...
    rating_scale_service = RatingScaleService(user=current_user_id)
    results = rating_scale_service.get_all_concepts(
        library=library,
        sort_by=sort_by,
...
def get_distinct_values_for_header(
    ...
    rating_scale_service = RatingScaleService(user=current_user_id)
    return rating_scale_service.get_distinct_values_for_header(
        library=library,
...
@router.get(
    "/rating-scales/{uid}",
    summary="Get details on a specific rating scale (in a specific version)",
    description="""
State before:
...
Possible errors:
 ...
 """,
    response_model=RatingScale,
    status_code=200,
    responses={500: {"model": ErrorResponse, "description": "Internal Server Error"}},
)
def get_activity(
    uid: str = RatingScaleUID, current_user_id: str = Depends(get_current_user_id)
):
    rating_scale_service = RatingScaleService(user=current_user_id)
    return rating_scale_service.get_by_uid(uid=uid)


@router.get(
    "/rating-scales/{uid}/versions",
    summary="List version history for rating scale",
    description="""
State before:
 - uid must exist.
...
Possible errors:
 - Invalid uid.
    """,
    response_model=List[RatingScale],
    status_code=200,
    responses={
        404: {
            "model": ErrorResponse,
            "description": "Not Found - The rating scale with the specified 'uid' wasn't found.",
        },
        500: {"model": ErrorResponse, "description": "Internal Server Error"},
    },
)
def get_versions(
    ...
    rating_scale_service = RatingScaleService(user=current_user_id)
    return rating_scale_service.get_version_history(uid=uid)


@router.post(
    "/rating-scales",
    summary="Creates new rating scale.",
    description="""
State before:
...
""",
    ...
    status_code=201,
    responses={
        201: {"description": "Created - The rating scale was successfully created."},
        403: {
            "model": ErrorResponse,
...
def create(
    rating_scale_create_input: RatingScaleCreateInput = Body(None, description=""),
    current_user_id: str = Depends(get_current_user_id),
):
    rating_scale_service = RatingScaleService(user=current_user_id)
    return rating_scale_service.create(concept_input=rating_scale_create_input)
...
State before:
 ...
 - The rating scale must belongs to a library that allows deleting (the 'is_editable' property of the library needs to be true).
...
def edit(
    uid: str = RatingScaleUID,
    rating_scale_edit_input: RatingScaleEditInput = Body(None, description=""),
    current_user_id: str = Depends(get_current_user_id),
):
    rating_scale_service = RatingScaleService(user=current_user_id)
    return rating_scale_service.edit_draft(
        uid=uid, concept_edit_input=rating_scale_edit_input
    )
