...
from consumer_api.shared import exceptions
...
class MockOauthClient:
    """Mock OauthClient for consumer_api.auth.jwk_service.JWKService"""
...
async def test_correct_test_setup(jwk_service, jwk_good_key, jwk_wrong_key):
    ...
    claims_in = mk_claims()
    log.debug(f"Creating token from claims: {claims_in!s}")
    token = mk_jwt(claims_in, jwk_good_key)

    log.info(f"Test JWT: {token!s}")

    log.debug("Decoding JWT")
    claims = jwt.decode(token, jwk_good_key)
    assert claims == claims_in, "Claims does not match after encoding-decoding cycle"

    log.debug(f"Validating claims: {claims!s}")
    claims.validate()
